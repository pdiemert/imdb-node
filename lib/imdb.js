...
var parseOptions = {
    ...
    elements: [
        ...
        },
	    {
		    name: 'votes',
		    sel: function($) {
			    return $('#overview-top .star-box span[itemprop="ratingCount"]').text().trim().replace(',','');
		    }
	    },
        {
            name: 'img',
...
module.exports = function (site, cb) {
    ...
    function movieData(imdbAddress) {
        parse(imdbAddress, jquery, function ($, err) {
            var result = {};
            result['imdbid'] = getId(imdbAddress);

		        for(var i=0; i<parseOptions.elements.length; i++)
		        {
			        var elem = parseOptions.elements[i];
    ...
    else if (site.substr(0,2) === 'tt')
    {
	    movieData('http://www.imdb.com/title/' + site);
    }
    else
    {
